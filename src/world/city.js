...
export class City {
  ...
  static _addTraffic(gl, scene, programInfo) {
    const vehicleModels = [
      "police", "ambulance", "taxi", "sedan", "suv",
      "firetruck", "van", "truck", "delivery",
    ];

    for (let i = 0; i < 22; i++) {
      const modelName = vehicleModels[Math.floor(Math.random() * vehicleModels.length)];
  ...
  static _addLights(gl, scene, programInfo) {
    ...
    for (let p = -140; p <= 140; p += 35) {
      ...
      postPositions.push([22, 7.5, p], [-22, 7.5, p], [p, 7.5, 22], [p, 7.5, -22]);
    }
  ...
  static _addBuildings(gl, scene, programInfo) {
    ...
    for (let x = -144; x <= 144; x += 32) {
      for (let z = -144; z <= 144; z += 32) {
        ...
        const reservedForLanding = Math.abs(x - 70) < 42 && Math.abs(z - 70) < 38;
        if (reservedForRoads || reservedForLanding || Math.random() < 0.16) continue;
        ...
        const footprint = isTower ? 17 + Math.random() * 5 : 14 + Math.random() * 6;
        const height = isTower ? 24 + Math.random() * 8 : 12 + Math.random() * 8;

        building.position = [x + Math.random() * 6 - 3, 0, z + Math.random() * 6 - 3];
  ...
  static _addProps(gl, scene, programInfo) {
    const props = [
      { path: "assets/models/antenna.obj", position: [-118, 0, 24], scale: [8, 8, 8], color: [0.82, 0.88, 0.94] },
      { path: "assets/models/antenna.obj", position: [124, 0, -28], scale: [8, 8, 8], color: [0.82, 0.88, 0.94] },
      { path: "assets/models/windmill.obj", position: [-152, 0, 116], scale: [12, 12, 12], color: [1.0, 0.94, 0.8] },
      { path: "assets/models/windmill.obj", position: [146, 0, -124], scale: [12, 12, 12], color: [1.0, 0.94, 0.8] },
      { path: "assets/models/tractor.obj", position: [-92, 0, -132], scale: [6, 6, 6], color: [0.95, 0.82, 0.42] },
      { path: "assets/models/detail-parasol-a.obj", position: [38, 0, 88], scale: [7, 7, 7], color: [1.0, 0.9, 0.65] },
      { path: "assets/models/detail-parasol-b.obj", position: [104, 0, 42], scale: [7, 7, 7], color: [0.84, 0.94, 1.0] },
      { path: "assets/models/detail-overhang-wide.obj", position: [-52, 0, 32], scale: [8, 8, 8], color: [1.0, 0.72, 0.52] },
      { path: "assets/models/detail-awning-wide.obj", position: [52, 0, -32], scale: [8, 8, 8], color: [0.72, 0.9, 1.0] },
    ];
