  ...
  for (const rawLine of lines) {
    ...
    if (cmd === 'v') {
      ...
      for (let i = 1; i < verts.length - 1; i++) {
        ...
        pushVertex(verts[i],   positions, texcoords, normals, outPos, outTex, outNorm);
        pushVertex(verts[i+1], positions, texcoords, normals, outPos, outTex, outNorm);
      }
    }
  }
...
function pushVertex(token, positions, texcoords, normals, outPos, outTex, outNorm) {
